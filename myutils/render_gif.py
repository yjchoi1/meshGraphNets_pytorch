...
flags.DEFINE_string("rollout_dir", "/work2/08264/baagee/frontera/meshnet/result/", help="Directory where rollout.pkl are located")
flags.DEFINE_string("rollout_name", "result0", help="Name of rollout `.pkl` file")
...
def render_gif_animation():
    ...
    ground_truth_vel = result[0][0]
    predicted_vel = result[0][1]
    ...
    triang = tri.Triangulation(result[1][:, 0], result[1][:, 1])

    # color
    vmin = np.concatenate(
        (predicted_vel[:, 0], ground_truth_vel[:, 0])).min()
    vmax = np.concatenate(
        (predicted_vel[:, 0], ground_truth_vel[:, 0])).max()
